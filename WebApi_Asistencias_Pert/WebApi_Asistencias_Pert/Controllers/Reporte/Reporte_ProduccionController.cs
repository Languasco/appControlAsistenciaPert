...
{
   [EnableCors("*", "*", "*")]
    public class Reporte_ProduccionController : ApiController
    {
       ...
       public object GetRegistro_Produccion(int opcion, string filtro)
       {
           ...
           try
           {
               ...
               else if (opcion == 3)
               {
                   ...
                   Reporte_Produccion_BL obj_negocio = new Reporte_Produccion_BL();
                   resul = obj_negocio.Listar_ReporteProduccion(local, OT_contable, fechaAsistencia, id_personalResponsable);
